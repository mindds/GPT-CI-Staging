...
def random_sampling_score(id_list_, data_, train_id_, test_id_, num_features_, save_path, n=10):
    """
    ...
    mat_avg = [{'Random_sampling':[]} for i in range(2)]  
...
if __name__ == "__main__":
    ...
    # for s in tqdm(stages, desc='outest0 loop',position=0):
    ...
    data_mapped = embedding_obj.mapped_data(data)

    folder_name_main = 'results-predictions-dementiabert-RS'
    if not os.path.exists(folder_name_main):
        os.makedirs(folder_name_main)

    num_features = 512
    res = random_sampling_score(id_list1, data_mapped, train_id1, test_id1, num_features, save_path=folder_name_main, n=10)
